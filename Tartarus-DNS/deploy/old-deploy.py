...
def _parse_options(args):
    ...
    parser.add_option("--nobcast", action="store_true",
            help="do not add reverse zones for broadcast "
                 "addresses (see RFC 1537)")
    parser.add_option("--nolocal", action="store_true",
            help="do not add local zones (like 'localhost.', see RFC 1537)")
    ...
    parser.add_option("-f", "--config-file", metavar="FILE",
            help="read options from file FILE")
    ...
    if opts['config_file']:
        # we'll re-use Ice Properties. Because we can.
        props = Ice.createProperties()
        props.load(opts['config_file'])
        # command-line options take preferance over config file options
        d = props.getPropertiesForPrefix('')
        d.update(opts)
        opts = d
